...
function Header() {
	...
	const { currentSinhvien, token, msg } = useSelector(
		(state) => state.sinhvien
	)
	...
	useEffect(() => {
		...
		isLoggedInSinhvien &&
			dispatch(actions.getListMonhocByMSSV(currentSinhvien?.mssv))
	...
	}, [isLoggedInSinhvien, listMH?.length, list?.length, token, msg])
		...
		<header className='w-full bg-white px-[28px] py-[12px] min-h-[66px] flex justify-between items-center border-b-2 border-solid'>
			<div className='flex items-center'>
				<Link to={linkRoute.HOME_SV}>
					<img
						className='w-[450px] h-[70px] rounded-sm p-1'
