...
function ResetMatkhau() {
   ...
   const handleSubmitChangePassword = () => {
      if (!newPass || !confirmPass) toast.error('Vui lòng nhập đầy đủ  thông tin...!')
      else if (newPass.newPass.length < 6) toast.error('Mật khẩu phải có ít nhất 6 ký tự...!')
      else {
         if (newPass.newPass !== confirmPass.confirmPass) toast.error('Mật khẩu không khớp...!')
         else {
            if (dispatch(actions.ResetPassWord(email.email?.slice(0), newPass))) {
   ...
   return (
      <main className="h-[755px] flex justify-center items-center">
         ...
         <div className="fixed w-[400px] min-h-[300px] bg-[#FFF] border-[#ccc] boder-solid border-[1px] rounded-xl p-4">
            ...
            <h4 className="text-center uppercase text-[32px] p-2">Reset Password</h4>
            <div className="flex- flex-col">
               <label htmlFor="old-pass" className="text-[#5a53bc] hover:underline cursor-pointer">
                  New password
               </label>
               <input
               ...
               />
               <label htmlFor="new-pass" className="text-[#5a53bc] hover:underline cursor-pointer">
                  Confirm password
               </label>
               <input
                  ...
                  type="password"
                  id="new-pass"
                  onChange={(e) => setConfirmPass({ confirmPass: e.target.value })}
               />
            </div>
            <Button
