						...
						{danhsachsvdk && danhsachsvdk.length > 0 ? (
							danhsachsvdk.map((item, index) => {
								return (
									<tr key={index} className='hover:bg-[#00000013]'>
										...
										{item.thu.split(", ").map((t, index) => {
											return (
												<tr key={index}>
													...
													<td width={74}>{item.phong}</td>
													<td width={100}>{item.tengiangvien}</td>
													<td width={300}>
										...
										<td
											width={240}
											className='hover:text-[#993333] hover:underline cursor-pointer'
										>
											Tài liệu
										</td>
									</tr>
							...
							<tr>
								<td colSpan={11} className='text-center'>
									Chưa đăng ký hoặc chưa lưu phiếu đăng ký môn học
									trong CSDL. Nhấn{" "}
									<Link
