...
function SuaMatkhau() {
   ...
   const handleEditPassword = () => {
      if (!editPass.oldPass && !editPass.newPass && !editPass.confirmPass) return
      else if (editPass.oldPass !== sinhvien.matkhau) {
         toast.error('Mật khẩu cũ không đúng...!')
      } else if (!editPass.newPass || !editPass.confirmPass) toast.error('Vui lòng nhập đầy đủ thông tin...!')
      else if (editPass.newPass === editPass.oldPass) toast.error('Mật khẩu mới phải khác mật khẩu cũ...!')
      else if (editPass.newPass.length < 6) toast.error('Mật khẩu phải có ít nhất 6 ký tự...!')
      else if (editPass.newPass !== editPass.confirmPass) toast.error('Mật khẩu xác nhận không trùng khớp')
      else {
            ...
            <>
               <div className="w-[30%] my-4 border-solid border-r-2 border-black text-center">
                  <img
                     ...
                     src={sinhvien?.avatar && require(`../../assets/images/${sinhvien?.avatar}`)}
                  />
               ...
               <div className="w-full p-4">
                  ...
                  <div className="flex flex-col w-[60%] text-[16px] pt-[16px] pb-[20px]">
                     <label className="font-bold cursor-pointer" htmlFor="old-pass">
                        Mật khẩu củ
                     </label>
                     <input
                        onChange={(e) => setEditPass({ ...editPass, oldPass: e.target.value })}
                  ...
                  <div className="flex flex-col w-[60%] text-[16px] pt-[16px] pb-[20px]">
                     <label className="font-bold cursor-pointer" htmlFor="new-pass">
                        Mật khẩu mới
                     </label>
                     <input
                        onChange={(e) => setEditPass({ ...editPass, newPass: e.target.value })}
                  ...
                  <div className="border-b-2 border-solid border-[#0000000d] flex flex-col w-[60%] text-[16px] pt-[16px]">
                     <label className="font-bold cursor-pointer" htmlFor="confirm-newPass">
